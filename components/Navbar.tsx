...
const CustomLink = ({href, className, children, setIsOpen}: linkProps) => {
	...
	return (
        ...
        ${pathname === href ? "text-primary dark:text-white lg:after:scale-x-100" : ""} ${className}`}
...
export default function Navbar() {
	...
	return (
      ...
      `}
		>
			<Link href={"/"}>
				<Image src={logo} alt='logo' width={75} unoptimized quality={100} loading='eager' priority className='lg:w-[85px] dark:hidden' />
				<Image src={logoWhite} alt='logo' width={75} unoptimized quality={100} loading='eager' priority className='lg:w-[85px] hidden dark:block' />
			</Link>
			<div>
				<button className='block lg:hidden' onClick={() => setIsOpen(true)} aria-label='Show navigation'>
          ...
          ${isOpen ? "translate-x-0" : "translate-x-[105%]"}`}
				>
					<button className='lg:hidden mb-3 ml-auto' onClick={() => setIsOpen(false)} aria-label='Close navigation'>
