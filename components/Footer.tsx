	...
	return (
		<footer className='relative bg-background border-t border-border mt-auto py-8 dark:text-slate-200 z-10'>
			<div className='flex flex-col md:flex-row justify-between md:items-center gap-8 text-sm font-medium'>
				<Image src={logo} alt='IEEE Logo' width={150} unoptimized quality={100} className='dark:hidden' />
				<Image src={logoWhite} alt='IEEE Logo' width={150} unoptimized quality={100} className='hidden dark:block' />

				<div className='grid grid-cols-2 place-items-between gap-y-3 gap-x-8 md:gap-x-14'>
					{links.map((link) => (
						<Link key={link.name} href={link.href} className='hover:translate-x-2 transition ease-linear dark:hover:text-white'>
			...
			<hr className='my-8' />
			<p className='text-xs font-medium text-slate-800 dark:text-slate-300'>&copy; 2025 IEEE SCU SB. All rights reserved.</p>
		</footer>
	);
}
