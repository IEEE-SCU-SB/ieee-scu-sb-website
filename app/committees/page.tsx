...
export default function Committees() {
	...
	const filteredCommittees = selectedCategory === "all" ? committees : committees.filter((committee) => committee.category === selectedCategory);

	return (
		<>
			<Header title='Explore Our Committees'>
				<p className='my-4 max-w-3xl mx-auto text-slate-800 dark:text-slate-200'>
					combining technical innovation with organizational excellence. Whether you&apos;re into coding, robotics, media, or event management,
				...
				<div className='flex gap-2 justify-center mb-6'>
					<button
						...
						className={`rounded-full px-3 py-1 text-sm font-medium border border-black dark:border-white ${
							selectedCategory === "all" ? "bg-black text-white dark:bg-white dark:text-black" : "bg-background text-foreground"
					...
					<button
						...
						className={`rounded-full px-3 py-1 text-sm font-medium border border-primary ${
							selectedCategory === "technical" ? "bg-primary text-white" : "bg-background text-primary"
					...
					<button
						...
						className={`rounded-full px-3 py-1 text-sm font-medium border border-purple ${
							selectedCategory === "non-technical" ? "bg-purple text-white" : "bg-background text-purple"
			...
			<main>
				<h2 className='sr-only'>
					{selectedCategory === "all" ? "All Committees" : selectedCategory === "technical" ? "Technical Committees" : "Non-Technical Committees"}
				</h2>

				<div>
					<div className='flex flex-wrap gap-8 justify-center'>
						{filteredCommittees.map(({id, category, title, subtitle, image}) => (
							<CommitteeCard key={id} id={id} category={category} title={title} subtitle={subtitle} image={image} />
						))}
					</div>
				</div>
