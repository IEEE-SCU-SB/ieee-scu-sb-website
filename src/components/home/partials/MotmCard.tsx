  ...
  return published && name !== "" ? (
    ...
    >
      ...
      <div className="w-full h-52 max-w-48 mx-auto md:mx-0 bg-gradient-to-bl from-[#b8860b] via-[#ffd700] to-[#b8860b] rounded-3xl shadow-lg overflow-hidden sm:order-2">
        {image && image.src !== "" && (
          <Image
            src={image.src}
            alt={image.alt}
            className="w-full h-full pt-4 object-cover object-top"
          />
        )}
      </div>
      ...
      <div className="flex flex-col items-center text-center p-3 gap-4 sm:items-start sm:text-left">
        <div className="">
          ...
          <p className="font-medium text-sm">{committee}</p>
        </div>

        {/* Bio */}
        <p className="max-w-[540px]">
          This month&apos;s spotlight shines on someone who went above and beyond.
          Thank you for your dedication and contributions to the team — you continue to inspire and
          raise the bar!
        </p>
        ...
        {linkedin && (
          <a href={linkedin} target="_blank">
            <svg
              className="fill-primary dark:fill-slate-200 mt-2 hover:translate-y-[2px] dark:hover:fill-white transition"
              width="16"
              height="16"
              viewBox="0 0 20 20"
              version="1.1"
    ...
    >
      ...
      <p className="max-w-sm font-medium my-auto text-center sm:text-left">
        The spotlight&apos;s waiting! Stay tuned for our next Member of the
        Month!
      </p>
    </div>
  );
