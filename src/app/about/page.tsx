				...
				<nav>
					<ul className='flex flex-wrap gap-2 justify-center mt-4'>
						{scrollLinks.map((link, index) => (
							<ScrollLink key={index} label={link.label} onClick={() => scrollToSection(link.ref)} />
						))}
					</ul>
				</nav>
			</Header>
				...
				<div className='relative grid gap-24 sm:gap-32 md:gap-44 max-w-3xl mx-auto translate-x-3 sm:translate-x-10'>
					...
					{aboutSections.map((section) => (
						<section
							...
							className={`px-4 sm:px-8 transition-opacity duration-300 ${activeSection === section.id ? "opacity-100" : "opacity-65"}`}
						>
							<Image src={section.logo.src} alt={section.logo.alt} width={95} unoptimized quality={100} className='dark:hidden' />
							{/* Dark mode Image */}
							<Image src={section.logoWhite.src} alt={section.logoWhite.alt} width={95} unoptimized quality={100} className='hidden dark:block' />
							...
							{section.learnMoreLink && (
								<a href={section.learnMoreLink.url} target='_blank' className='text-primary text-sm inline-block mt-3 font-medium group'>
